...
class FruitClassifier:
    ...
    def Model(self, InternalActivation, activation, Padding, DropOutValue):
        self.fruitClassifierModel = tf.keras.Sequential([
    ...
    def TrainModel(self, Epochs, Optimizer):
        ...
        self.fruitClassifierModel.compile(
            optimizer = Optimizer,
            loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits = True),
            metrics = ['accuracy']
        )

        self.fruitClassifierModel.summary()

        self.History = self.fruitClassifierModel.fit(
            self.train_dataset,
    ...
    def Result(self, PercentageLimiter):
        if (self.percentage_Confidence < PercentageLimiter):
            print("Unknown type!")
            print("This Fruit is most likely to be {} with a {:.2f} percent confidence." .format(self.classNames[np.argmax(self.score)], 100 * np.max(self.score)))
        else:
            print("This Bird is most likely to be {} with a {:.2f} percent confidence." .format(self.classNames[np.argmax(self.score)], 100 * np.max(self.score)))
    
    def SaveModel(self, ModelName):
        converter = tf.lite.TFLiteConverter.from_keras_model(self.fruitClassifierModel)
